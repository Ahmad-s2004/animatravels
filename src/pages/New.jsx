...
const Light = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans antialiased selection:bg-sky-500 selection:text-white overflow-x-hidden">
          ...
          <div className="lg:col-span-6 text-left space-y-6">
            ...
            <div className="bg-white p-2.5 rounded-2xl border border-slate-200/80 shadow-sm max-w-lg flex flex-col sm:flex-row gap-2">
              <div className="flex-grow flex items-center gap-2 px-3 py-1.5">
                <MapPin className="text-sky-500 flex-shrink-0" size6={16} />
      ...
      {/* 3. ACTIVE DEPARTURES HUB (FIXED MOBILE FILTER OVERFLOW) */}
      <section className="py-20 max-w-7xl mx-auto px-5 sm:px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          ...
          {/* Mobile-Optimized Horizontal Scroll Filter System */}
          <div className="w-full md:w-auto overflow-x-auto -mx-5 px-5 md:mx-0 md:px-0 scrollbar-none">
            <div className="flex bg-slate-100 p-1.5 rounded-xl w-max min-w-full md:min-w-0 gap-1">
              {categories.map(cat => (
                <button
                  key={cat.id}
                  onClick={() => setActiveTab(cat.id)}
                  className={`text-xs font-bold px-4 py-2 rounded-lg transition-all duration-200 whitespace-nowrap ${
                    activeTab === cat.id ? 'bg-white text-sky-600 shadow-sm' : 'text-slate-500 hover:text-slate-900'
                  }`}
                >
                  {cat.name}
                </button>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
          ...
          <div className="space-y-2">
            ...
            <div className="flex bg-slate-50 border border-slate-200 rounded-xl p-1">
              ...
              <button className="bg-sky-600 text-white p-2 rounded-lg"><Send size={11}/></button>
            </div>
          </div>
        </div>
